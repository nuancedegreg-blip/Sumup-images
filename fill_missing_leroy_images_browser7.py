...
AUDIT=Path('verified_leroy_images_audit.csv'); STATE=Path('verified_leroy_images_state_browser7.json')
...
def open_product(page,pid,url):
    try:
        page.goto(url,wait_until='domcontentloaded',timeout=45000); accept_cookies(page); page.wait_for_timeout(2500)
    except Exception: return False
...
def rendered_image(page,pid):
    ...
    try: page.wait_for_selector('img.mc-carousel-main__image, .mc-carousel-main img, img[src*="media.adeo.com"], source[srcset*="media.adeo.com"]',timeout=15000)
    except Exception: pass
    page.wait_for_timeout(1000)
    # First use the actual image selected and loaded by Chromium.
    for sel in ['img.mc-carousel-main__image','.mc-carousel-main img','img[src*="media.adeo.com"]']:
        loc=page.locator(sel)
        for i in range(min(loc.count(),80)):
            try:
                data=loc.nth(i).evaluate("el => ({src:el.currentSrc||el.src||'',w:el.naturalWidth||0,h:el.naturalHeight||0,complete:!!el.complete})")
            except Exception: continue
    ...
    # If the image element is lazy-loaded, use the first ADEO srcset selected by the product carousel.
    for sel in ['.mc-carousel-main source','picture source[srcset*="media.adeo.com"]','source[srcset*="media.adeo.com"]']:
        loc=page.locator(sel)
        for i in range(min(loc.count(),80)):
            try: val=loc.nth(i).get_attribute('srcset') or ''
            except Exception: continue
            for part in val.split(','):
                u=html.unescape(part.strip().split()[0] if part.strip() else '')
                if is_adeo(u): return u,'verified_srcset'
...
print(f'BROWSER7 START blank={len(missing)} pending={len(pending)} batch={len(batch)}',flush=True)
if not batch:
