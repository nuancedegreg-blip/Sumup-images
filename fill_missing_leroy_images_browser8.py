...
STATE = Path('verified_leroy_images_state_browser8.json')
...
def load_state():
    ...
    try:
        ...
        return x if isinstance(x, dict) else {'processed': [], 'completed': False}
    except Exception:
        return {'processed': [], 'completed': False}
...
def score_image(url):
    try:
        u = urlparse(url)
        if (u.hostname or '').lower() != 'media.adeo.com' or '/media/' not in u.path:
            return -1
        q = parse_qs(u.query)
        w = int((q.get('width') or q.get('w') or ['0'])[0] or 0)
        h = int((q.get('height') or q.get('h') or ['0'])[0] or 0)
        if 0 < w < 250 or 0 < h < 250:
            return -1
        if any(x in url.lower() for x in ['home-index', 'logo', 'flag', 'picto', 'icon']):
            return -1
...
def capture_product_image(page, pid, url):
    ...
    def on_response(resp):
        try:
            ...
            if 'media.adeo.com/media/' in u and resp.status == 200:
                ...
                if ctype.startswith('image/') and score_image(u) >= 0:
                    hits.append(u)
        except Exception:
            pass
    ...
    try:
        ...
        if pid not in page.url:
            return '', 'product_open_failed'

        # Read rendered src/srcset/currentSrc as a second source, matching the user's DevTools view.
        try:
            values = page.locator('img, source').evaluate_all("""els => els.flatMap(el => [el.currentSrc || '', el.src || '', el.getAttribute('srcset') || '', el.getAttribute('data-src') || '', el.getAttribute('data-srcset') || ''])""")
            for value in values:
                if not value:
                    continue
                for part in str(value).split(','):
                    candidate = part.strip().split()[0] if part.strip() else ''
                    if 'media.adeo.com/media/' in candidate:
                        hits.append(html.unescape(candidate))
        except Exception:
            pass
    ...
    for u in hits:
        ...
        if u not in seen and score_image(u) >= 0:
            ...
            uniq.append(u)
    if not uniq:
        return '', 'no_adeo_image'
    uniq.sort(key=score_image, reverse=True)
    return uniq[0], 'verified_headful'
...
missing = [
    ...
    if SKU_RE.fullmatch((row.get('SKU') or '').strip())
    and not (row.get('Image 1') or '').strip()
]
pending = [row for row in missing if (row.get('SKU') or '').strip() not in processed]
...
with sync_playwright() as p:
    ...
    for i, row in enumerate(batch, 1):
        sku = (row.get('SKU') or '').strip()
        pid = sku[3:]
        product_url, source = exact_url(page, pid)
        img = ''
        status = source
        if product_url:
            img, image_status = capture_product_image(page, pid, product_url)
...
if FINAL.exists():
    ...
    for row in final_rows:
        sku = (row.get('SKU') or '').strip()
        if SKU_RE.fullmatch(sku) and not (row.get('Image 1') or '').strip() and sku in image_map:
...
remaining_blank = sum(1 for row in master if SKU_RE.fullmatch((row.get('SKU') or '').strip()) and not (row.get('Image 1') or '').strip())
remaining_unprocessed = sum(1 for row in master if SKU_RE.fullmatch((row.get('SKU') or '').strip()) and not (row.get('Image 1') or '').strip() and (row.get('SKU') or '').strip() not in processed)
