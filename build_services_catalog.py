...
# Prestations génériques : prix libre selon chantier.
SERVICES = [
...
# Tarifs issus du catalogue maître du Projet : prix achat HT Leroy Merlin.
# On génère T10 et T20 avec le prix TTC correspondant pour un ajout direct au devis.
LM_RATES = [
...
for category, name, sku, unit in SERVICES:
    ...
    row[DESC] = f'Prestation de main-d’œuvre - unité : {unit} - tarif à adapter au chantier'
    rows.append(row)

for family, name, code, ht, unit, detail in LM_RATES:
    for vat in (10, 20):
        row = blank_row()
        ttc = round(ht * (1 + vat / 100), 2)
        row['Item name'] = f'LM - {name} - TVA {vat}%'
        row['Price'] = f'{ttc:.2f}'
        row['Variable price? (Yes/No)'] = 'No'
        row['Tax rate (%)'] = f'{vat:.2f}'
        row['SKU'] = f'LM-{code}-T{vat}'
        row['Category'] = f'Barème LM - {family}'
        if 'Unit' in row:
            row['Unit'] = unit
        row['Display item at Checkout? (Yes/No)'] = 'Yes'
        row['Display item in Online Store? (Yes/No)'] = 'No'
        row['Track inventory? (Yes/No)'] = 'No'
        row[DESC] = f'Tarif LM HT : {ht:.2f} € | {detail} | Barème Leroy Merlin {code} | Unité : {unit}'
        rows.append(row)
...
print(f'{len(rows)} prestations générées dans {OUT} dont {len(LM_RATES) * 2} lignes tarifées Leroy Merlin')
