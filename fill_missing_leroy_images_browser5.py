...
STATE = Path('verified_leroy_images_state_browser5.json')
...
def exact_leroy_url_from_search(page, pid):
    # Real web search: only the 8-digit reference + Leroy Merlin.
    ...
    for search_url in engines:
        ...
        # Then inspect final rendered HTML, including wrapped search URLs.
        try:
...
def open_exact_product(page, pid, url):
    try:
...
def extract_main_image(page, pid):
    ...
    for u in candidates:
        ...
        if u in seen or not is_adeo(u): continue
        seen.add(u)
        if image_opens(u): return u, 'verified_browser5'
...
print(f'BROWSER5 START blank={len(missing)} pending={len(pending)} batch={len(batch)}', flush=True)
if not batch:
...
with sync_playwright() as p:
    ...
    for i, row in enumerate(batch, 1):
        ...
        product_url, source = exact_leroy_url_from_search(page, pid)
        img = ''; status = source
        if product_url and open_exact_product(page, pid, product_url):
            img, istatus = extract_main_image(page, pid); status = istatus + ':' + source
        elif product_url:
            status = 'product_open_failed:' + source
        if img:
        ...
        processed.add(sku)
        print(f'{i}/{len(batch)} {sku} status={status} url={product_url} newly_verified={verified_now}', flush=True)
